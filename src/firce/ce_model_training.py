...
def train_ce_binary(
    ...
    match config.model_variant:
        ...
        case ModelVariant.FEEDFORWARD:
            ...
            logger.debug(
                f'[feedforward] Converted data to tensors: X_tensor.shape={tuple(X_tensor.shape)}, Y_tensor.shape={tuple(Y_tensor.shape)}'
            )
            ...
            if device.type == 'mps':
                ...
                logger.debug('Eval on CUDA (per-batch sigmoid on CUDA, then move to CPU)')
                with torch.no_grad():
                    for i, (xb, _) in enumerate(eval_loader):
                        logger.debug(f'Eval batch {i} start: xb_device={xb.device}, xb_shape={tuple(xb.shape)}')
                ...
                with torch.no_grad():
                    for i, (xb, _) in enumerate(eval_loader):
                        ...
                        logger.debug(
                            f'Eval batch {i} after cast/contig: xb_device={xb.device}, xb_dtype={xb.dtype}, xb_is_contig={xb.is_contiguous()}'
                        )
                        logits_i = model(xb)  # (B,)
                        logger.debug(f'Eval batch {i} logits shape={tuple(logits_i.shape)}')
