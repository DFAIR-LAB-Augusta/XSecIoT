...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--shap', action='store_true', help='Enable SHAP for XAI')
    parser.add_argument('--lime', action='store_true', help='Enable LIME for XAI')
    return parser.parse_args()


def _explain_with_lime(
    model, X_train, X_test, feature_names, class_names, outputPath, output_prefix='lime_plot'
) -> None:
    explainer = LimeTabularExplainer(
        training_data=X_train, feature_names=feature_names, class_names=class_names, mode='classification'
    )
    exp = explainer.explain_instance(X_test[0], model.predict_proba)
    exp.save_to_file(os.path.join(outputPath, f'{output_prefix}_instance.html'))
    print(f'LIME explanation saved to {os.path.join(outputPath, f"{output_prefix}_instance.html")}')


def _explain_with_shap(
    model, X_sample, outputPath, feature_names, model_type='tree', output_prefix='shap_plot'
) -> None:
    """
    ...
    if isinstance(shap_values, list) and len(shap_values) > 1:
        for i, class_shap_values in enumerate(shap_values):
            plt.figure()
            shap.summary_plot(class_shap_values, X_sample, feature_names=feature_names, show=False)
            plt.tight_layout()
            plt.savefig(os.path.join(outputPath, f'{output_prefix}_class{i}summary.png'))
            plt.close()
            print(f'SHAP summary for class {i} {os.path.join(outputPath, f"{output_prefix}_class{i}_summary.png")}')
    else:
        plt.figure()
        ...
        plt.close()
        print(f'SHAP summary saved to {os.path.join(outputPath, f"{output_prefix}_summary.png")}')
...
def run_binary_classification(aggregated_file: str, isUNSW: bool, isPCA: bool) -> None:
    ...
    if isPCA:
        cv_scores = cross_val_score(rf, X_pca, y, cv=5, scoring='accuracy')  # type: ignore
    else:
    ...
    if isPCA:
        cv_scores = cross_val_score(knn, X_pca, y, cv=5, scoring='accuracy')  # type: ignore
    else:
    ...
    if isPCA:
        cv_scores = cross_val_score(dt, X_pca, y, cv=5, scoring='accuracy')  # type: ignore
    else:
    ...
    _explain_with_shap(
        rf,
        X_test[:100],
        outputPath=xai_output_dir,
        model_type='tree',
        output_prefix='rf_binary',
        feature_names=feature_names,
    )
    _explain_with_lime(
        rf, X_train, X_test, feature_names, class_names, outputPath=xai_output_dir, output_prefix='rf_binary'
    )
    ...
    _explain_with_shap(
        ...
        outputPath=xai_output_dir,
        model_type='tree',
        output_prefix='dt_binary',
        feature_names=feature_names,
    )
    _explain_with_lime(
        dt, X_train, X_test, feature_names, class_names, outputPath=xai_output_dir, output_prefix='dt_binary'
    )
...
def run_multiclass_classification(aggregated_file: str, isUNSW: bool, isPCA: bool) -> None:
    ...
    if isPCA:
        cv_scores = cross_val_score(rf_multi, X1_pca, y1, cv=5, scoring='accuracy')  # type: ignore
    else:
        cv_scores = cross_val_score(rf_multi, X1_final, y1, cv=5, scoring='accuracy')
    ...
    if isPCA:
        cv_scores = cross_val_score(knn_multi, X1_pca, y1, cv=5, scoring='accuracy')  # type: ignore
    else:
        cv_scores = cross_val_score(knn_multi, X1_final, y1, cv=5, scoring='accuracy')
    ...
    if isPCA:
        cv_scores = cross_val_score(dt_multi, X1_pca, y1, cv=5, scoring='accuracy')  # type: ignore
    else:
        cv_scores = cross_val_score(dt_multi, X1_final, y1, cv=5, scoring='accuracy')
    ...
    if isPCA:
        cv_scores = cross_val_score(svm_multi, X1_pca, y1, cv=5, scoring='accuracy')  # type: ignore
    else:
        cv_scores = cross_val_score(svm_multi, X1_final, y1, cv=5, scoring='accuracy')
    ...
    xgb_multi = xgb.XGBClassifier(
        objective='multi:softmax', num_class=len(label_encoder.classes_), eval_metric='mlogloss', random_state=42
    )
    xgb_multi.fit(X_train_multi, y_train_multi)
    if isPCA:
        cv_scores_xgb = cross_val_score(xgb_multi, X1_pca, y1_encoded, cv=5, scoring='accuracy')  # type: ignore
    else:
        cv_scores_xgb = cross_val_score(xgb_multi, X1_final, y1_encoded, cv=5, scoring='accuracy')
    ...
    print(
        classification_report(
            y_test_labels, y_pred_labels, labels=present_labels, target_names=[str(label) for label in present_labels]
        )
    )
    ...
    feedforward_model.fit(
        X_train_nn, y_train_nn, epochs=20, batch_size=32, validation_data=(X_test_nn, y_test_nn), verbose=0
    )
    ...
    print(
        classification_report(y_test_ff, y_pred_ff, labels=sorted(np.unique(y_test_ff)), target_names=target_names_ff)
    )
    ...
    feature_names_multi = X1.columns.tolist() if not isPCA else [f'f{i}' for i in range(X1_final.shape[1])]
    class_names_multi = list(label_encoder.classes_) if isUNSW else sorted(data['Label'].unique())
