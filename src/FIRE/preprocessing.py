...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--unsw', action='store_true', help='Enable UNSW-specific preprocessing pipeline')
    ...
    if is_unsw:
        ...
        for col in ['tot_fwd_pkts', 'tot_bwd_pkts', 'totlen_fwd_pkts', 'totlen_bwd_pkts']:
            if col not in data.columns:
                raise ValueError(f'Missing required column after renaming: {col}')

        data['fwd_pkt_len_mean'] = np.where(
            data['tot_fwd_pkts'] != 0, data['totlen_fwd_pkts'] / data['tot_fwd_pkts'], 0
        )
        data['bwd_pkt_len_mean'] = np.where(
            data['tot_bwd_pkts'] != 0, data['totlen_bwd_pkts'] / data['tot_bwd_pkts'], 0
        )
        total_pkts = data['tot_fwd_pkts'] + data['tot_bwd_pkts']
        data['pkt_len_mean'] = np.where(
            total_pkts != 0, (data['totlen_fwd_pkts'] + data['totlen_bwd_pkts']) / total_pkts, 0
        )
        data['flow_iat_mean'] = np.where(total_pkts > 1, data['flow_duration'] / (total_pkts - 1), 0)
        data['down_up_ratio'] = np.where(data['tot_fwd_pkts'] > 0, data['tot_bwd_pkts'] / data['tot_fwd_pkts'], 0)
...
def _aggregate_sessions(data: pd.DataFrame, is_unsw: bool) -> pd.DataFrame:
    ...
    if is_unsw:
        ...
        session['total_packets'] = session['total_forward_packets'] + session['total_backward_packets']
        session['total_bytes'] = session['total_bytes_forward'] + session['total_bytes_backward']
        return session
    else:
        session = data.groupby(['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol']).agg(
            flow_duration=('flow_duration', 'sum'),
            total_forward_packets=('tot_fwd_pkts', 'sum'),
            total_backward_packets=('tot_bwd_pkts', 'sum'),
            total_bytes_forward=('totlen_fwd_pkts', 'sum'),
...
def _sliding_window_aggregation(
    data: pd.DataFrame, window_size: pd.Timedelta, step_size: pd.Timedelta, is_unsw: bool
) -> pd.DataFrame:
    """
    ...
    if is_unsw:
        meta = pd.DataFrame(
            columns=[
                'start_time',
                'end_time',
                'total_forward_packets_window',
                'total_backward_packets_window',
                'total_forward_bytes_window',
                'total_backward_bytes_window',
                'average_packet_size_fwd_window',
                'average_packet_size_bwd_window',
                'flow_duration_window',
                'packet_count_window',
                'mean_iat_fwd_window',
                'stddev_iat_fwd_window',
                'min_iat_fwd_window',
                'max_iat_fwd_window',
                'mean_iat_bwd_window',
                ...
                'min_iat_bwd_window',
                'max_iat_bwd_window',
                'flow_rate_packets_window',
                'flow_rate_bytes_window',
                'flow_direction_ratio_window',
                'byte_direction_ratio_window',
                'src_ip_entropy_window',
                'dst_ip_entropy_window',
                'protocol',
                'src_ip',
                'dst_ip',
                'src_port',
        ...
        meta = meta.astype({
            ...
            'mean_iat_fwd_window': 'float64',
            'stddev_iat_fwd_window': 'float64',
            'min_iat_fwd_window': 'int64',
            'max_iat_fwd_window': 'int64',
            'mean_iat_bwd_window': 'float64',
            'stddev_iat_bwd_window': 'float64',
            'min_iat_bwd_window': 'int64',
            'max_iat_bwd_window': 'int64',
            'flow_rate_packets_window': 'float64',
            'flow_rate_bytes_window': 'float64',
            'flow_direction_ratio_window': 'float64',
            'byte_direction_ratio_window': 'float64',
            'src_ip_entropy_window': 'float64',
            'dst_ip_entropy_window': 'float64',
            'protocol': 'int64',
            'src_ip': 'object',
            'dst_ip': 'object',
            'src_port': 'int64',
        ...
        records = []
        times = pd.date_range(start=data.index.min(), end=data.index.max(), freq=step_size)
        for st in times:
            et = st + window_size
            ...
            dur = (window['timestamp'].max() - window['timestamp'].min()).total_seconds() + 1e-9
            rec = {
                'start_time': st,
                'end_time': et,
                'total_forward_packets_window': window['tot_fwd_pkts'].sum(),
...
def _merge_aggregated_data(
    sliding_data: pd.DataFrame, session_data: pd.DataFrame, original_data: pd.DataFrame, is_unsw: bool
) -> pd.DataFrame:
    if is_unsw:
        ...
        print(f'Session merge done in {time.time() - t0:.2f}s', file=sys.stderr)
        orig = original_data.reset_index()
        subset = orig[['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol', 'Label', 'Attack']].drop_duplicates()
        merged = merged.merge(subset, on=['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol'], how='left')
        if 'start_time_x' in merged.columns:
            first = merged['start_time_x'].min()
            merged['timestamp_offset_seconds'] = (merged['start_time_x'] - first).dt.total_seconds()
        else:
        ...
        subset = original_data[['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol', 'Label']].drop_duplicates()
        merged = merged.merge(subset, on=['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol'], how='left')
        return merged


def _preprocess_pipeline(
    file_path: str, window_size_str: str = '5s', step_size_str: str = '1s', is_unsw: bool = False
) -> pd.DataFrame:
    """
    ...
    if is_unsw:
        print(f'Full data time range: {data.index.min()} to {data.index.max()}', file=sys.stderr)
        print(
            f'[{time.strftime("%H:%M:%S")}] Calling sliding_window_aggregation (UNSW)...', file=sys.stderr, flush=True
        )
    sliding_data = _sliding_window_aggregation(data, window_size, step_size, is_unsw)
    if is_unsw:
        print(
            f'[{time.strftime("%H:%M:%S")}] DONE: 3. Compute sliding window aggregation (UNSW).',
            file=sys.stderr,
            flush=True,
        )
    else:
        print('DONE: 3. Compute sliding window aggregation.')
    aggregated_data = _merge_aggregated_data(sliding_data, session_data, data, is_unsw)
    if is_unsw:
        print('DONE: 4. Merge the aggregated results (UNSW).', file=sys.stderr, flush=True)
    else:
...
def run_preprocessing(
    ...
    """
    aggregated_data = _preprocess_pipeline(file_path, window_size_str, step_size_str, is_unsw)
...
if __name__ == '__main__':
    ...
    run_preprocessing(args.file_path, args.window_size, args.step_size, args.unsw)
