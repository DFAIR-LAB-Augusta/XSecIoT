...
class InductiveConformalEvaluator:
    ...
    def calibrate(
        self, X: np.ndarray, y: np.ndarray, perf_stats: PerformanceStats, significance: Optional[float] = None
    ) -> None:
        """
        ...
        X_train, X_calib, y_train, y_calib = train_test_split(
            X, y, test_size=self.calibration_split, random_state=self.random_state, stratify=y
        )
        ...
        scores = compute_nonconformity_scores(probas, y_calib, self.model.classes_)

        self.calibration_scores = {cls: scores[y_calib == cls] for cls in np.unique(y_calib)}
        ...
        if self.significance_controller:
            ...
            self.thresholds = compute_class_thresholds(self.calibration_scores, sig)
    ...
    def predict_p_values(self, X: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        if self.calibration_scores is None:
            raise RuntimeError('ICE must be calibrated before computing p-values.')

        probas = self.model.predict_proba(X)
        preds = self.model.predict(X)
        scores = 1.0 - np.array([probas[i, np.where(self.model.classes_ == preds[i])[0][0]] for i in range(len(preds))])
