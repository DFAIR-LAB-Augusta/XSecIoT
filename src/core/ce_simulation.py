...
def _simulate(
    ...
    elif config.model_type == ModelType.MULTI and config.is_unsw:
    ...
    elif config.is_unsw and config.model_type == ModelType.MULTI:
        rolling_cols = [
            'totlen_bwd_pkts',
            'tot_bwd_pkts',
            'totlen_fwd_pkts',
    ...
    with LoggerCls(log_dir, max_rows=config.max_rows, columns=rolling_cols) as rolling:
        ...
        vals = seed_df['BinLabel'] if config.model_type == ModelType.BINARY else seed_df['MC_Label']
        logger.debug(f'[pre-clean] Label dtype={vals.dtype}, n_rows={len(vals)}')
        logger.debug(f'[pre-clean] Label nunique(excl NaN)={vals.nunique(dropna=True)}, n_nan={int(vals.isna().sum())}')
        uniques = pd.unique(vals)
        logger.debug(f'[pre-clean] Label unique values (raw): {list(uniques)}')
        ...
        if isinstance(rolling, CircularDequeLogger) and config.use_mlp and config.is_unsw:
            ...
            vals = df_log['BinLabel'] if config.model_type == ModelType.BINARY else df_log['MC_Label']

            logger.debug(
                f'[pre-clean] Label dtype={vals.dtype}, n_rows={len(vals)}',
                f'[pre-clean] Label nunique(excl NaN)={vals.nunique(dropna=True)}, n_nan={int(vals.isna().sum())}',
            )
        ...
        if config.ce_type != CEType.NONE:
            ...
            if config.is_unsw:
                if config.model_type == ModelType.BINARY:
                    ...
                    logging.debug(f"UNSW y classes: {clean_tr['MC_Label'].unique() = }")

            ytr = clean_tr['BinLabel'] if config.model_type == ModelType.BINARY else clean_tr['MC_Label']
            ...
            if config.use_svm:
                ...
                if config.model_type == ModelType.BINARY:
                    ...
                    ce_kwargs.pop('threshold', None)

                    ce_model = MLP_CE_Multiclass(
                        input_dim=input_dim,
                    ...
                    logging.info(
                        f'Using MLPMulticlassCE for CE on device={config.device} '
                        f'(input_dim={input_dim}, num_classes={num_classes}, classes={classes.tolist()})'
                    )
...
def _log_results(config: SimulationConfig, overall: float, perf_stats: PerformanceStats) -> None:
    ...
    if perf_stats.correct_log:
        ...
        if config.use_adaptive_chunking:
            plot_path = (
                log_dir
                / 'ac'
                / f'{config.model_variant.value}_{config.ce_type.value}_{config.model_type.value}_accuracy_plot.png'
            )
        else:
            plot_path = (
                log_dir
        ...
        if perf_stats.drift_intervals:
            ...
            if config.use_adaptive_chunking:
                plot_path = (
                    log_dir
                    / 'ac'
                    / f'{config.model_variant.value}_{config.ce_type.value}_{config.model_type.value}_drift_intervals.png'  # noqa: E501
                )
            else:
                plot_path = (
                    log_dir
            ...
            if config.use_adaptive_chunking:
                hist_path = (
                    log_dir
                    / 'ac'
                    / f'{config.model_variant.value}_{config.ce_type.value}_{config.model_type.value}_drift_interval_histogram.png'  # noqa: E501
                )
            else:
                hist_path = (
                    log_dir
    ...
    if perf_stats.ce_stats.accuracies:
        ...
        ce_metric_plot = (
            plot_dir
            / f'{config.model_variant.value}_{config.ce_type.value}_{config.model_type.value}_ce_training_metrics.png'
        )
    ...
    if perf_stats.chunk_sizes and config.use_adaptive_chunking:
        ...
        chunk_plot_path = (
            chunk_plot_dir
            / f'{config.model_variant.value}_{config.ce_type.value}_{config.model_type.value}_chunk_size_trace.png'
        )
