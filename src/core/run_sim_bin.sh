...
for cs in "${CHUNK_SIZES[@]}"; do
  for model in "${MODEL_VARIANTS[@]}"; do
    for ce in "${CE_TYPES[@]}"; do
      ...
      # --- MLP CE Chunk Size ---
      COMMAND="PYTHONPATH='.' caffeinate uv run src/core/ce_simulation.py datasets/CETrain/combined_data.csv datasets/CEFlows2/CE_Bin_Flows2_merged.csv --log2File --modelVariant $model --ceType $ce --max_rows 100000 --useCircularLogger --debug --useMLP --chunk_size $cs " # DFAIR

      # COMMAND="PYTHONPATH='.' caffeinate uv run src/core/ce_simulation.py datasets/UNSW_NB15/NF-UNSW-NB15-v3.csv datasets/CEFlows2/CE_Bin_Flows2_merged.csv --log2File --modelVariant $model --ceType $ce --max_rows 100000 --useCircularLogger --debug --useMLP --chunk_size $cs --unsw" # UNSW_NB15

      # COMMAND="PYTHONPATH='.' caffeinate uv run src/core/ce_simulation.py datasets/CIC_UNSW/NF-CICIDS2018-v3.csv datasets/CEFlows2/CE_Bin_Flows2_merged.csv --log2File --modelVariant $model --ceType $ce --max_rows 100000 --useCircularLogger --debug --useMLP --chunk_size $cs --unsw" # CICIDS2018
