...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--unsw', action='store_true', help='Run in UNSW‐dataset mode')
    parser.add_argument('--pca', action='store_true', help='Enable PCA for feature reduction')
    parser.add_argument('--noPre', action='store_true', help='Skip preprocessing step')
    parser.add_argument('--noMod', action='store_true', help='Skip modeling step')
    parser.add_argument('--noSim', action='store_true', help='Skip simulations step')
...
def main() -> None:
    ...
    aggregated = os.path.join(os.path.dirname(args.dataset_path), 'aggregated_data.csv')

    # 1) Preprocessing
    if not args.noPre:
        print('=== Preprocessing ===')
        run_preprocessing(args.dataset_path, args.window_size, args.step_size, args.unsw)

    # 2) Modeling
    if not args.noMod:
        if args.noPre and not os.path.exists(aggregated):
            print(f'Error: {aggregated} not found (used --noPre)', file=sys.stderr)
            sys.exit(1)
