...
CE_BLOCK = re.compile(r'ce_stats\s*=\s*ModelStats\((?P<inside>.*?)\)', re.DOTALL)
CLF_BLOCK = re.compile(r'classifier_stats\s*=\s*ModelStats\((?P<inside>.*?)\)', re.DOTALL)
...
MODELVAR_RE = re.compile(r'\bmodelVariant\s*=\s*([A-Za-z0-9_\-]+)', re.IGNORECASE)
...
def _parse_both_series(line: str) -> Optional[Dict[str, Dict[str, List[float]]]]:
    ...
    if m_clf:
        ...
        logging.warning('Perf stats found, but classifier_stats block missing.')
...
def _scan_logs_group_by_classifier(
    ...
    for fpath in files:
        ...
        logging.info('Parsed CE+Classifier for %s | classifier=%s, ce=%s', run_rel, classifier, ce_type)
        ...
        for row_idx, (metric_key, metric_label) in enumerate(METRICS):
            ...
            for _run_label, series_pair in bucket.items():
                ...
                if isinstance(ce_series, dict):
                    ...
                    if y:
                        any_data = True
                        y_plot = [(v / 100.0) if metric_is_percent[metric_key] else v for v in y]
                        x = list(range(1, len(y_plot) + 1))
                        ax.plot(x, y_plot, linestyle='-', marker='.', linewidth=1, label=None)

                if isinstance(clf_series, dict):
                    y2 = clf_series.get(metric_key, [])
                    if y2:
                        any_data = True
                        y2_plot = [(v / 100.0) if metric_is_percent[metric_key] else v for v in y2]
                        x2 = list(range(1, len(y2_plot) + 1))
                        ax.plot(x2, y2_plot, linestyle='--', marker='x', linewidth=1, label=None)
            ...
            if col_idx == 0:
                ylabel = metric_label + (' (proportion of 1.0)' if metric_is_percent[metric_key] else '')
...
def _write_classifier_csv(
    ...
    with out_csv.open('w', encoding='utf-8', newline='') as fh:
        w = csv.writer(fh)
        w.writerow(['classifier', 'series', 'run', 'ce_type', 'metric', 'step', 'value'])
        for ce_key in CE_ORDER:
            bucket = runs_by_ce.get(ce_key, {})
                ...
                for series_name in ('ce', 'classifier'):
                    ...
                    for metric_key, _metric_label in METRICS:
                        ...
                        for i, v in enumerate(y, start=1):
                            w.writerow([classifier, series_name, run_label, ce_key, metric_key, i, v])
